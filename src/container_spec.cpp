...
bool valid_container_name(std::string_view value)
{
    ...
    const auto valid_rest = [](unsigned char c) {
...
{
    return !value.empty() && !has_control(value);
}
...
Result<void> validate_container_spec(const ContainerSpec& spec)
{
    if (!valid_container_name(spec.name))
    {
        return std::unexpected(make_error(ErrorKind::invalid_argument,
                                          "container name must match [A-Za-z0-9][A-Za-z0-9_.-]{0,127}"));
    }
    if (spec.image.empty() || has_control(spec.image))
    {
        return std::unexpected(make_error(ErrorKind::invalid_argument,
                                          "container image must be non-empty and contain no control characters"));
    }
    ...
    for (const auto& [key, value] : spec.env)
    {
        ...
        {
            return std::unexpected(make_error(ErrorKind::invalid_argument,
                                              "environment keys must be non-empty and contain no control characters"));
        }
    }
    ...
    for (const auto& [key, value] : spec.labels)
    {
        ...
        {
            return std::unexpected(make_error(ErrorKind::invalid_argument,
                                              "label keys must be non-empty and contain no control characters"));
        }
    }
        ...
        if (spec.resource_limits->cpu)
        {
            ...
            if (spec.resource_limits->cpu->cpus && !valid_cpuset(*spec.resource_limits->cpu->cpus))
            {
                return std::unexpected(make_error(ErrorKind::policy,
                                                  "cpu.cpus must contain only digits, commas, and dashes"));
            }
        }
