...
Result<HttpResponse> finish_perform(CURL* curl, HttpResponse response, CURLcode rc)
{
    if (rc != CURLE_OK)
    {
        return std::unexpected(make_error(ErrorKind::transport,
                                          std::string{"curl request failed: "} + curl_easy_strerror(rc),
                                          0,
                                          0,
                                          static_cast<int>(rc)));
    }
...
Result<HttpResponse> require_success(Result<HttpResponse> response, std::string_view operation)
{
    ...
    {
        return std::unexpected(make_error(ErrorKind::http,
...
PodmanClient::PodmanClient(PodmanTarget target, ClientOptions options)
    : target_{std::move(target)}
    , options_{std::move(options)}
{
}

const PodmanTarget& PodmanClient::target() const noexcept
{
    return target_;
}
...
Result<HttpResponse> PodmanClient::list_containers(bool all,
                                                   const std::vector<std::string>& label_filters) const
{
...
Result<HttpResponse> PodmanClient::stop_container(std::string_view name, int timeout_seconds) const
{
    ...
    HttpRequest req;
    req.method = "POST";
    req.path = versioned_path("/libpod/containers/" + encoded_name +
                              "/stop?timeout=" + std::to_string(timeout_seconds));
...
Result<HttpResponse> PodmanClient::remove_container(std::string_view name, bool force) const
{
    ...
    HttpRequest req;
    req.method = "DELETE";
    req.path = versioned_path("/libpod/containers/" + encoded_name + "?force=" +
                              (force ? "true" : "false"));
...
Result<HttpResponse> PodmanClient::load_image_archive(const std::filesystem::path& archive_path) const
{
    ...
    return require_success(finish_perform(curl.get(), std::move(response), curl_easy_perform(curl.get())),
                           "load image archive");
}
}
