...
Result<void> validate_podman_socket(const PodmanTarget& target,
                                    const SocketValidationOptions& options)
{
    ...
    if (options.require_default_path)
    {
        ...
        if (actual != expected)
        {
            return std::unexpected(make_error(ErrorKind::socket_validation,
                                              "target socket path '" + actual.string() +
    ...
    {
        return std::unexpected(make_error(ErrorKind::socket_validation,
                                          "lstat failed for '" + target.socket_path.string() + "': " +
                                              std::strerror(errno),
                                          0,
                                          errno));
    }
    ...
    {
        return std::unexpected(make_error(ErrorKind::socket_validation,
                                          "target socket path is a symlink: " + target.socket_path.string()));
    }

    if (options.require_socket && !S_ISSOCK(st.st_mode))
    {
        return std::unexpected(make_error(ErrorKind::socket_validation,
                                          "target path is not a Unix socket: " + target.socket_path.string()));
    }

    if (options.require_owner && st.st_uid != target.uid)
    {
        return std::unexpected(make_error(ErrorKind::socket_validation,
                                          "target socket owner uid " + std::to_string(st.st_uid) +
