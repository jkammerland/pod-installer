...
{
std::filesystem::path RuntimeDirectoryLayout::runtime_dir_for(uid_t uid) const
{
    return root / std::to_string(uid);
}

std::filesystem::path RuntimeDirectoryLayout::podman_socket_for(uid_t uid) const
{
    return runtime_dir_for(uid) / "podman" / "podman.sock";
}

PodmanTarget make_target(uid_t uid,
...
Result<PodmanTarget> resolve_user(std::string_view user_name,
                                  RuntimeDirectoryLayout layout,
                                  std::string api_version)
{
    if (user_name.empty())
    ...
    if (rc != 0)
    {
        return std::unexpected(make_error(ErrorKind::filesystem,
                                          "getpwnam_r failed for user '" + user_name_owned + "': " +
                                              std::strerror(rc),
                                          0,
                                          rc));
    }
    if (result == nullptr)
    {
        return std::unexpected(make_error(ErrorKind::invalid_argument,
                                          "user '" + user_name_owned + "' does not exist"));
    }

    return make_target(pw.pw_uid, pw.pw_name, std::move(layout), std::move(api_version));
}

Result<PodmanTarget> resolve_uid(uid_t uid, RuntimeDirectoryLayout layout, std::string api_version)
{
    passwd pw{};
    passwd* result{};
    std::vector<char> buffer(passwd_buffer_size());
    ...
    {
        return std::unexpected(make_error(ErrorKind::filesystem,
                                          "getpwuid_r failed for uid " + std::to_string(uid) + ": " +
                                              std::strerror(rc),
                                          0,
                                          rc));
    }
    ...
    {
        return std::unexpected(make_error(ErrorKind::invalid_argument,
                                          "uid " + std::to_string(uid) + " does not exist"));
    }
